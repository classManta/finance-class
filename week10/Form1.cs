...
namespace COMSample1
{
    public partial class Form1 : Form
    {
        ...
        class KLineBuilder
        {
            private KLine current = null;
            private ListBox listTicks2;

            public KLineBuilder(ListBox listBox)
            {
                this.listTicks2 = listBox;
            }

            public void OnTick(string stockNo, DateTime tickTime, double price, double qty)
            {
                DateTime barEnd = GetBarEndTime(tickTime);

                // 第一筆
                if (current == null)
                {
                    current = NewBar(stockNo, barEnd, price, qty);
                    return;
                }

                // 跨K（含跳空）
                while (barEnd > current.Time)
                {
                    Output(current);

                    DateTime nextEnd = current.Time.AddMinutes(1);

                    if (barEnd > nextEnd)
                    {
                        // 補空K
                        current = EmptyBar(stockNo, nextEnd, current.Close);
                    }
                    else
                    {
                        current = NewBar(stockNo, barEnd, price, qty);
                ...
                // 同一根K
                Update(current, price, qty);
            }

            private DateTime GetBarEndTime(DateTime t)
            {
                return new DateTime(t.Year, t.Month, t.Day, t.Hour, t.Minute, 0)
                    .AddMinutes(1);
            }

            private KLine NewBar(string stockNo, DateTime end, double price, double qty)
            {
                return new KLine
                {
                    StockNo = stockNo,
            ...
            private KLine EmptyBar(string stockNo, DateTime end, double lastClose)
            {
                return new KLine
                {
                    StockNo = stockNo,
            ...
            private void Update(KLine bar, double price, double qty)
            {
            ...
            private void Output(KLine bar)
            {
                string strData =
                    $"{bar.StockNo}," +
                    $"{bar.Time:yyyyMMdd HH:mm:ss}," +
                    $"{bar.Open}," +
                    $"{bar.High}," +
                    $"{bar.Low}," +
                    $"{bar.Close}," +
                ...
                string msg = "[KLINE]" + strData;

                // UI安全
                if (listTicks2.InvokeRequired)
                {
                    listTicks2.Invoke(new Action(() =>
                    {
                        listTicks2.Items.Add(msg);
                        listTicks2.TopIndex = listTicks2.Items.Count - 1;
                    }));
                }
                else
                {
        ...
        DateTime ParseDateTime(int date, int time, int micro)
        {
            string d = date.ToString();
            string t = time.ToString().PadLeft(6, '0');
            string m = micro.ToString().PadLeft(6, '0');
            ...
            // 加上毫秒（取前3碼）
            int ms = int.Parse(m.Substring(0, 3));
        ...
        void m_SKQuoteLib_OnNotifyHistoryTicks(short sMarketNo, string strStockNo, int nPtr, int nDate, int lTimehms, int lTimemillismicros, int nBid, int nAsk, int nClose, int nQty, int nSimulate)
        {
            ...
            DateTime tickTime = ParseDateTime(nDate, lTimehms, lTimemillismicros);

            builder.OnTick(strStockNo, tickTime, nClose, nQty);
        }
        ...
        KLineBuilder builder;
        public Form1()
        {
            InitializeComponent();

            builder = new KLineBuilder(listTicks2);
        ...
        public List<KDResult> CalculateKD(
    List<KLine> klines,
    int rsvPeriod,
    int kPeriod,
    int dPeriod)
        {
            List<KDResult> results = new List<KDResult>();
            ...
            for (int i = 0; i < klines.Count; i++)
            {
                ...
                var window = klines.Skip(i - rsvPeriod + 1).Take(rsvPeriod);

                double highest = window.Max(x => x.High);
                double lowest = window.Min(x => x.Low);
                double close = klines[i].Close;
                ...
                if (results.Count > 0)
                {
                    ...
                    if (prev.K.HasValue && prev.D.HasValue)
                    {
                        if (prev.K < prev.D && k > d)
                            signal = "GoldenCross";
                        else if (prev.K > prev.D && k < d)
                            signal = "DeathCross";
                    }
                }
        ...
        private void buttonCalculateKD_Click(object sender, EventArgs e)
        {
            string stockNo = textBoxKDStockNo.Text.Trim();
            ...
            if (!int.TryParse(textBoxRSVPeriod.Text, out int rsvPeriod) || rsvPeriod <= 0)
            {
                MessageBox.Show("請輸入正確的 RSV 期間");
                return;
            }

            if (!int.TryParse(textBoxK.Text, out int kPeriod) || kPeriod <= 0)
            {
                MessageBox.Show("請輸入正確的 K 平滑參數");
                return;
            }

            if (!int.TryParse(textBoxD.Text, out int dPeriod) || dPeriod <= 0)
            {
                MessageBox.Show("請輸入正確的 D 平滑參數");
                return;
            }
            ...
            var klines = m_KLineData[stockNo].Values.ToList();

            if (klines.Count < rsvPeriod)
            {
                MessageBox.Show($"K線資料不足，至少需要 {rsvPeriod} 筆");
                return;
            }

            var results = CalculateKD(klines, rsvPeriod, kPeriod, dPeriod);

            StringBuilder sb = new StringBuilder();

            foreach (var r in results)
            {
                if (!r.K.HasValue) continue;

                sb.AppendLine($"{r.Time:yyyy/MM/dd}  RSV={r.RSV:F2}  K={r.K:F2}  D={r.D:F2}  {r.Signal}");
            }

            richTextBoxKD.Text = sb.ToString();
        }
    }
}
